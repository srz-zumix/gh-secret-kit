...
| `--dst-token` | *（通常不要）* コピー先用 PAT を保持するコピー元リポジトリのシークレット変数名（例: `DST_PAT`；ワークフロー内で `${{ secrets.DST_PAT }}` として参照される）。ランナーがコピー先ホストに対して `gh` 認証済みでない場合にのみ使用します |
...
| `--dst-token` | *（通常不要）* コピー先用 PAT を保持するコピー元リポジトリのシークレット変数名（例: `DST_PAT`）。ランナーがコピー先ホストに対して `gh` 認証済みでない場合にのみ使用します |
