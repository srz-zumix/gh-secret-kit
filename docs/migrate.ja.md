...
> **注意**: `migrate` は Dependabot シークレットをサポートしていません。Dependabot シークレットは Dependabot によってトリガーされたワークフローからしかアクセスできません。コピーする場合は [`gh secret-kit secret dependabot copy`](commands/secret.md#secret-dependabot-copy) を使用してください。
...
- [コマンドリファレンス](commands/README.md) — 各コマンドの使い方・引数・オプション
- [`migrate` コマンドリファレンス](commands/migrate.md)
