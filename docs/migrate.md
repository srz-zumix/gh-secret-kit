...
> **Note**: Dependabot secrets are **not** supported by `migrate`. Dependabot secrets can only be accessed by workflows triggered by Dependabot. Use [`gh secret-kit secret dependabot copy`](commands/secret.md#secret-dependabot-copy) to copy them instead.
