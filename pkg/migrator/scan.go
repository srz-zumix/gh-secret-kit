...
// EnvMatch represents a matched environment pair with its secret and variable metadata.
...
func ScanMatchingRepos(ctx context.Context, src, dst *OrgContext) ([]RepoMatch, error) {
	...
	for _, srcRepo := range repos {
		...
		if len(srcEnvSecrets) > 0 || len(srcEnvVariables) > 0 {
			...
			// Collect all env names from both secrets and variables.
			envNameSet := make(map[string]bool)
			for envName := range srcEnvSecrets {
				envNameSet[envName] = true
			}
			for envName := range srcEnvVariables {
				envNameSet[envName] = true
			}

			for envName := range envNameSet {
				srcEnvSecs := srcEnvSecrets[envName]
				var envSecretNames []string
				for _, s := range srcEnvSecs {
					envSecretNames = append(envSecretNames, s.Name)
				}
				srcEnvVars := srcEnvVariables[envName]
				var envVariableNames []string
				for _, v := range srcEnvVars {
					envVariableNames = append(envVariableNames, v.Name)
				}
				envMatches = append(envMatches, EnvMatch{
					Name:          envName,
					SecretCount:   len(srcEnvSecs),
					SecretNames:   envSecretNames,
					VariableCount: len(srcEnvVars),
					VariableNames: envVariableNames,
...
func collectEnvDataWithReviewers(
...
) (map[string][]*github.Secret, map[string][]*github.ActionsVariable, map[string]bool, error) {
	...
	envVariables := make(map[string][]*github.ActionsVariable)
	...
	for _, env := range envs {
		...
		variables, err := gh.ListEnvVariables(ctx, g, repoRef, name)
		if err != nil {
			return nil, nil, nil, fmt.Errorf("failed to list variables for environment %s: %w", name, err)
		}
		if len(variables) > 0 {
			envVariables[name] = variables
		}
	}
	return envSecrets, envVariables, envReviewers, nil
}
