...
func NewDumpCmd() *cobra.Command {
	...
	f := cmd.Flags()
	f.StringVarP(&config.Source, "repo", "R", "", "Source repository (e.g., owner/repo; defaults to the repository running the workflow; when set, enables target-specified mode)")
	...
	f.StringVar(&config.RunnerLabel, "runner-label", types.DefaultRunnerLabel, "Runner label for runs-on (matches the label used by migrate runner setup; required when --repo is specified)")
